/*
 * Strands — from React Bits (https://reactbits.dev/animations/strands)
 * Copyright (c) 2026 David Haz. MIT + Commons Clause.
 *
 * Vendored rather than pulled at runtime because the site's CSP only allows
 * scripts from its own origin. Modified in three places, each marked SHARP AI:
 * the licence permits modification; it forbids selling or redistributing the
 * component itself, which this project does not do — the site is free and
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    // SHARP AI 1/3: a full-screen WebGL loop that never stops is a battery
    // cost on a page people leave open. It runs only while the hero is on
    // screen and the tab is in front, and under prefers-reduced-motion it
    // paints a single frame and stops — a still field, not a blank hole.
    const reduced = matchMedia('(prefers-reduced-motion: reduce)').matches;
    let visible = true;
    let animateId = 0;
    const update = (t: number) => {
      if (!reduced && visible && !document.hidden) animateId = requestAnimationFrame(update);
    ...
    // SHARP AI 2/3: restart the loop when the tab or the hero comes back.
    const wake = () => {
